...
% % % % Creates Timeseries of the GFW projections averaged across a domain
    ...
    Regional_Energy_BORN_INT(:,i) = ((vecnorm(Proj_INT)*SC_U_INT')').^2;
    Regional_Energy_BORN_DEG(:,i) = ((vecnorm(Proj_DEG)*SC_U_DEG')').^2;
    Regional_Energy_BORN_SEG(:,i) = ((vecnorm(Proj_SEG)*SC_U_SEG')').^2;


    Proj_BORN(:,:,i) = TS*SC_U_matched_born_norm;
    Proj_BORN_Energy(:,i) = (vecnorm(Proj_BORN(:,:,i))').^2;
...
for i = 1:3
    for j = 1:3
        [~, p_mat(i,j)] = ttest2(data2plot(:,i), data2plot(:,j));
    end
end
...
%% % Ratio to Maximum Harmonic Entropy Figure
